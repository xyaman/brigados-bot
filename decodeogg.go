...
func DecodeBuffered(dst io.Writer, src io.Reader, closeCh chan struct{}) error {
	buf := bufio.NewReaderSize(src, MaxPageSize)
	return DecodeBuf(dst, buf, make([]byte, MaxPageSize), closeCh)
}

// DecodeBuf decodes with a custom buffer. The buffer must be at least
// MaxPageSize.
func DecodeBuf(dst io.Writer, src io.Reader, buf []byte, closeCh chan struct{}) error {
	if len(buf) < MaxPageSize {
		return errors.New("buffer is too small")
	}

	err := decode(dst, src, buf, closeCh)
	if err == io.EOF {
		return nil
	}
	return err
...
func decode(dst io.Writer, src io.Reader, buffer []byte, closeCh chan struct{}) error {
	var (
	...
	for {
		...
		for {
			...
			select {
			case <-closeCh:
				return fmt.Errorf("Forced to stop")
			default:
			}
